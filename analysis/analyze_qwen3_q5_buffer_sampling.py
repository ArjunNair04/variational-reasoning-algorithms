#!/usr/bin/env python3
"""Analyse the prespecified seven-seed Q5 buffer-sampling study."""
...
from generate_qwen3_17b_q5_buffer_sampling import (  # noqa: E402
    CELL_ORDER,
    CHECKPOINTS,
    CONTROL_CELL,
    RUN_ID,
...
PAIRWISE = (
    ("Q5-S32-B32-U1-FULL", CONTROL_CELL, "b32_full_vs_b16_full"),
    ("Q5-S32-B32-U1-MS16", "Q5-S32-B32-U1-FULL", "b32_sample16_vs_b32_full"),
    ("Q5-S32-B32-U1-MS16", CONTROL_CELL, "b32_sample16_vs_b16_full"),
)
...
def validate_design(config_path: Path) -> tuple[dict[str, Any], list[Any]]:
    ...
    if configured != CELL_ORDER or tuple(config["defaults"]["seed_values"]) != SEEDS:
        raise ValueError("Q5 buffer-sampling coordinates changed")
    return config, cells
...
def main() -> int:
    ...
    parser.add_argument("--q5-more-seed-metrics", type=Path)
    ...
    required = (
        ...
        "base_seed_metrics", "q5_more_seed_metrics", "output_dir",
    )
    ...
    controls = pd.read_csv(args.q5_more_seed_metrics)
    if "CTRL-base" not in set(bases["cell"]) or CONTROL_CELL not in set(controls["cell"]):
        raise ValueError("analysis inputs lack the registered base or Q5-MORE control")
    ...
    contrasts = paired_contrasts(
        results,
        controls.loc[controls["cell"] == CONTROL_CELL],
    ...
    analysis = {
        ...
        "historical_control": CONTROL_CELL,
