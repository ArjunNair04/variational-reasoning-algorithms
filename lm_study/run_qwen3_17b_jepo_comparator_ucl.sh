...
#$ -l tmem=24G
#$ -t 1-7
#$ -cwd -j y -o logs/qwen3_jepo_comparator.$JOB_ID.$TASK_ID.log
...
export YAML=${YAML:-$PROJ/lm_study/experiments_qwen3_17b_jepo_comparator.yaml}
...
cell_count=1
seed_count=7
test "$task_id" -ge 1 -a "$task_id" -le 7 || exit 4
seed_index=$((task_id - 1))

echo "== JEPO comparator | seed $((seed_index + 1))/$seed_count | $(hostname) | $(date) =="
