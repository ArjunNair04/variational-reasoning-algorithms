...
#$ -l h_rt=04:00:00
#$ -l tmem=8G
#$ -cwd -j y -o logs/qwen3_q5_support_followup_validate.$JOB_ID.log
...
"$VENV/bin/python" generate_qwen3_17b_q5_support_followup.py --check "$YAML"
"$VENV/bin/python" validate_qwen3_17b_q5_support_followup.py \
  --config "$YAML" \
  --log-glob "$PAYLOAD_PROJ/lm_study/logs/qwen3_q5_support_followup.${SOURCE_JOB_ID}.*.log" \
  --marker "$MARKER" \
