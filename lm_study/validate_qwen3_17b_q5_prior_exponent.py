#!/usr/bin/env python3
"""Fail-closed validator for the 12-task Q5 prior-exponent study."""
...
from generate_qwen3_17b_q5_prior_exponent import CELL_ORDER, RUN_ID, SEEDS, build_payload
...
def _validate_receipts(config: dict, result_root: Path, expected_commit: str) -> list[str]:
    problems = []
    ...
    for coordinate in _expected_coordinates(config):
        ...
        from analyze_qwen3_q5_prior_exponent import mechanism_rows
        from analyze_qwen3_jepo_comparator import _artifact, _read_json, _read_jsonl_gz, _validate_evaluation
        try:
            result = _read_json(_artifact(result_root, receipt, "cell_result_"))["result"]
            commit = json.loads(result["params"])["env"]["commit"]
            if len(commit) < 7 or not expected_commit.startswith(commit):
                raise ValueError("receipt execution commit mismatch")
            _validate_evaluation(_artifact(result_root, receipt, "eval_"))
            cell_index = (coordinate["task_id"] - 1) // len(SEEDS)
            tau = config["algos"]["AC-ALG1"][cell_index]["responsibility_prior_exponent"]
            mechanism_rows(_read_jsonl_gz(_artifact(result_root, receipt, "training_diagnostics_")), tau)
            if int(result["optimizer_steps"]) != 32 or int(result["train_llm_gen"]) != 2048:
                raise ValueError("training budget mismatch")
        except (KeyError, TypeError, OSError, ValueError) as exc:
            problems.append(f"{receipt_path}: {exc}")
        for prefix in (
            "cell_result_", "eval_", "prompt_contract_", "passk_",
            "checkpoint_eval_", "training_diagnostics_", "traj_",
        ):
...
def validate_study(
    ...
    *,
    ...
    problems.extend(_validate_receipts(config, result_root, expected_commit))
    marker = {
        "schema_version": 1,
        "status": "ok",
        "run_id": RUN_ID,
