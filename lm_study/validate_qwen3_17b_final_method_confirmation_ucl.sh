...
#$ -l h_rt=04:00:00
#$ -l tmem=8G
#$ -cwd -j y -o logs/qwen3_final_method_confirmation_validate.$JOB_ID.log
...
python generate_qwen3_17b_final_method_confirmation.py --check "$YAML"
python validate_qwen3_17b_final_method_confirmation.py \
  --config "$YAML" \
  --log-glob "$PAYLOAD_PROJ/lm_study/logs/qwen3_final_method_confirmation.${SOURCE_JOB_ID}.*.log" \
  --marker "$MARKER" \
