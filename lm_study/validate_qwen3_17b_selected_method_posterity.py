#!/usr/bin/env python3
"""Fail-closed validator for the 91-task selected-method posterity replay."""
...
def _validate_logs(pattern: str) -> list[str]:
    ...
    if len(paths) != 91:
        return [f"expected 91 payload logs, found {len(paths)}"]
    observed_tasks = []
    for path in paths:
    ...
    if sorted(observed_tasks) != list(range(1, 92)):
...
def _validate_receipts(config: dict, result_root: Path) -> list[str]:
    ...
    if len(receipt_paths) != 91:
        problems.append(f"expected 91 completion receipts, found {len(receipt_paths)}")
    fingerprints = set()
    trained_adapters = 0
    ...
    if trained_adapters != 84:
        problems.append(f"expected 84 trained adapters, found {trained_adapters}")
    return problems
