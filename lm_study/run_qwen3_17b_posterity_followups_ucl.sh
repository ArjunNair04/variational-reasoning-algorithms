...
#$ -l tmem=24G
#$ -t 1-42
#$ -cwd -j y -o logs/qwen3_posterity_followups.$JOB_ID.$TASK_ID.log
...
export YAML=${YAML:-$PROJ/lm_study/experiments_qwen3_17b_posterity_followups.yaml}
...
echo "== posterity follow-up | cell $((cell_index + 1))/$cell_count | seed $((seed_index + 1))/$seed_count | $(hostname) | $(date) =="
