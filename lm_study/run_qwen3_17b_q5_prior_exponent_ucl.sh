...
#$ -l tmem=24G
#$ -t 1-12
#$ -cwd -j y -o logs/qwen3_q5_prior_exponent.$JOB_ID.$TASK_ID.log
...
export PYTHONPATH="$PROJ/src:$PROJ/lm_study:${PYTHONPATH:-}"
export HF_HOME=${HF_HOME:-$VRL_ROOT/hf_cache}
export YAML=${YAML:-$PROJ/lm_study/experiments_qwen3_17b_q5_prior_exponent.yaml}
...
cell_count=4
seed_count=3
test "$task_id" -ge 1 -a "$task_id" -le 12 || exit 4
zero_index=$((task_id - 1))
cell_index=$((zero_index / seed_count))
seed_index=$((zero_index % seed_count))

echo "== Q5 prior exponent | cell $((cell_index + 1))/$cell_count | seed $((seed_index + 1))/$seed_count | $(hostname) | $(date) =="
