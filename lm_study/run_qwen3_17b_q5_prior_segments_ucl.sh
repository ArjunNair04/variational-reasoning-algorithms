...
#$ -l tmem=24G
#$ -t 1-9
#$ -cwd -j y -o logs/qwen3_q5_prior_segments.$JOB_ID.$TASK_ID.log
...
export YAML=${YAML:-$PROJ/lm_study/experiments_qwen3_17b_q5_prior_segments.yaml}
...
cell_count=3
seed_count=3
test "$task_id" -ge 1 -a "$task_id" -le 9 || exit 4
zero_index=$((task_id - 1))
cell_index=$((zero_index / seed_count))
seed_index=$((zero_index % seed_count))

echo "== Q5 prior segments | cell $((cell_index + 1))/$cell_count | seed $((seed_index + 1))/$seed_count | $(hostname) | $(date) =="
