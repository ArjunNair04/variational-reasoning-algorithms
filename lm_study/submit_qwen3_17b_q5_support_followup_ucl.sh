...
YAML=${YAML:-$SCRIPT_DIR/experiments_qwen3_17b_q5_support_followup.yaml}
RESULT_OUT=${RESULT_OUT:-$HOME/po_results/2026-09-03/q5-support-followup/qwen3-q5-support-followup__6f89a2c1}
MARKER=${MARKER:-$HOME/po_results/auto_state/qwen3_q5_support_followup_6f89a2c1.ok}
CONTROL_MARKER=${CONTROL_MARKER:-$HOME/po_results/auto_state/qwen3_q5_buffer_sampling_1d5b0eb4.ok}
CLAIM_DIR=${CLAIM_DIR:-$HOME/po_results/auto_state/qwen3_q5_support_followup_20260903.submit.claim}
...
required = {"status": "ok", "run_id": "1d5b0eb4", "task_count": 14, "official_test_used": False}
...
"$VENV/bin/python" -m py_compile \
  ...
  "$SCRIPT_DIR/generate_qwen3_17b_q5_support_followup.py" \
  "$SCRIPT_DIR/validate_qwen3_17b_q5_support_followup.py" \
  "$PROJ/analysis/analyze_qwen3_q5_support_followup.py"
bash -n "$SCRIPT_DIR/run_qwen3_17b_q5_support_followup_ucl.sh"
bash -n "$SCRIPT_DIR/validate_qwen3_17b_q5_support_followup_ucl.sh"
"$VENV/bin/python" "$SCRIPT_DIR/generate_qwen3_17b_q5_support_followup.py" --check "$YAML"
...
for cell in 0 1; do
  for seed in $(seq 0 6); do
    "$VENV/bin/python" "$SCRIPT_DIR/run_yaml.py" "$YAML" \
      --dry-run --seed "$seed" --shard "$cell" --nshard 2 \
...
payload_output=$(qsub -h \
  ...
  run_qwen3_17b_q5_support_followup_ucl.sh)
...
validator_output=$(qsub -hold_jid "$payload_job" \
  ...
  validate_qwen3_17b_q5_support_followup_ucl.sh 2>&1)
validator_status=$?
set -e
