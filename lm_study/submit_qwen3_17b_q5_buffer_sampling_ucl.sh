...
YAML=${YAML:-$SCRIPT_DIR/experiments_qwen3_17b_q5_buffer_sampling.yaml}
RESULT_OUT=${RESULT_OUT:-$HOME/po_results/2026-09-02/q5-buffer-sampling/qwen3-q5-buffer-sampling__1d5b0eb4}
MARKER=${MARKER:-$HOME/po_results/auto_state/qwen3_q5_buffer_sampling_1d5b0eb4.ok}
CONTROL_MARKER=${CONTROL_MARKER:-$HOME/po_results/auto_state/qwen3_posterity_followups_f20c9e17.ok}
CLAIM_DIR=${CLAIM_DIR:-$HOME/po_results/auto_state/qwen3_q5_buffer_sampling_20260902.submit.claim}
...
required = {"status": "ok", "run_id": "f20c9e17", "task_count": 42, "official_test_used": False}
...
"$VENV/bin/python" -m py_compile \
  ...
  "$SCRIPT_DIR/generate_qwen3_17b_q5_buffer_sampling.py" \
  "$SCRIPT_DIR/validate_qwen3_17b_q5_buffer_sampling.py" \
  "$PROJ/analysis/analyze_qwen3_q5_buffer_sampling.py"
bash -n "$SCRIPT_DIR/run_qwen3_17b_q5_buffer_sampling_ucl.sh"
bash -n "$SCRIPT_DIR/validate_qwen3_17b_q5_buffer_sampling_ucl.sh"
"$VENV/bin/python" "$SCRIPT_DIR/generate_qwen3_17b_q5_buffer_sampling.py" --check "$YAML"
...
payload_output=$(qsub -h \
  ...
  run_qwen3_17b_q5_buffer_sampling_ucl.sh)
...
validator_output=$(qsub -hold_jid "$payload_job" \
  ...
  validate_qwen3_17b_q5_buffer_sampling_ucl.sh 2>&1)
validator_status=$?
set -e
